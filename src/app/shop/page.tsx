    ...
    <Layout>
      <section className="bg-[linear-gradient(30deg,var(--color-red1),var(--color-purple2),var(--color-blue2))] w-full py-0 overflow-hidden">
        <div className="py-4 grid md:grid-cols-2 gap-6 float-right">
          {user ? (
            <Link href="/dashboard" className="bg-white text-black px-6 py-3 rounded-lg font-semibold hover:bg-gray-800">Dashboard</Link>
          ) : (
            <>
              <Link href="/register" className="bg-white text-black px-6 py-3 rounded-lg font-semibold hover:bg-gray-800">Register</Link>
              <Link href="/login" className="bg-white text-black px-6 py-3 rounded-lg font-semibold hover:bg-gray-800">Login</Link>
            </>
          )}
        </div>
      </section>
            ...
            {cart.map((item) => (
              <li key={item.id} className="flex justify-between items-center">
                ...
                <div className="flex items-center space-x-2">
                  <button onClick={() => decreaseQuantity(item.id)} className="px-2 py-1 bg-gray-200 text-black rounded hover:bg-gray-300">−</button>
                  <button onClick={() => addToCart(item)} className="px-2 py-1 bg-gray-200 text-black rounded hover:bg-gray-300">+</button>
                  <button onClick={() => removeFromCart(item.id)} className="px-2 py-1 bg-red-500 text-white rounded hover:bg-red-600">Remove</button>
                </div>
              </li>
            ))}
          </ul>

          <button onClick={handleCheckout} className="px-6 py-3 bg-green-600 text-white rounded hover:bg-green-700">
            Checkout
          </button>
        </section>
      )}
      ...
      {/* Suspense wrapper for safe useSearchParams */}
